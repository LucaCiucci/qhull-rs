...
fn main() {
    let convex_hull = Qh::builder().build_from_iter(points()).unwrap();

    let simplices = convex_hull.simplices().count();
    eprintln!(
        "Convex hull has {simplices} segments and {} vertices",
    ...
    let triangles = triangulation
        .simplices()
        .filter(|s| !s.upper_delaunay())
        .count();
    eprintln!(
        "Triangulation has {triangles} triangles and {} vertices",
    ...
    for (i, point) in points().enumerate() {
        ...
        doc = doc.add(
            svg::node::element::Text::new(format!("{i}").as_str())
                .set("x", point[0] + 2.0)
                .set("y", point[1])
                .set("font-size", 5)
                .set("fill", "black"),
...
fn points() -> impl Iterator<Item = [f64; 2]> {
    ...
    (0..10).map(move |_| [rng.gen_range(5.0..95.0), rng.gen_range(5.0..95.0)])
}
